...
exp(glm.fit2$coefficients) #Odds ratios

#marginal effects
LogitScalar <- mean(dlogis(predict(glm.fit2, type = "link")))
LogitScalar * coef(glm.fit2)

#predicted probabilities
plogit <- predict(glm.fit2, type="response")
summary(plogit)
...
loessfit2 <- loess(glm.fit2$fitted.values ~ nox,degree=1,span=2/3)
xx <- seq(.35,.9,0.05)
summary(glm.fit2)
par(mfrow=c(1,2))
...
plot(nox,glm.fit2$fitted.values,ylab=expression(hat(Y)),xlab="nox, x1")
...
#Using the bestglm package
```{r}
library(bestglm)
bm <- bestglm(crime_data, family = binomial, IC = "AIC")
```


#Predicting the results (bestglm)
```{r}

#Prediction model
pm2 <- predict(bm[[1]],crime_eval_data, type="response")
summary(pm)
```
